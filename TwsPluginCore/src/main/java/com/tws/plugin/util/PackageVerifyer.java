...
public class PackageVerifyer {
	...
	public static Signature[] collectCertificates(String sourcePath, boolean simpleMode) {
		...
		try {
			...
			if (simpleMode) {
				...
				if (certs == null) {
					QRomLog.e(TAG, "Package " + " has no certificates at entry " + jarEntry.getName() + "; ignoring!");
					jarFile.close();
					return null;
				}
				if (DEBUG) {
				...
				while (entries.hasMoreElements()) {
					...
					if (localCerts == null) {
						QRomLog.e(TAG, "Package " + " has no certificates at entry " + je.getName() + "; ignoring!");
						jarFile.close();
						return null;
					} else if (certs == null) {
						certs = localCerts;
						...
						for (int i = 0; i < certs.length; i++) {
							...
							if (!found || certs.length != localCerts.length) {
								QRomLog.e(TAG, "Package " + " has mismatched certificates at entry " + je.getName()
										+ "; ignoring!");
								jarFile.close();
								return null;
							}
			...
			if (certs != null && 0 < certs.length) {
				...
				QRomLog.e(TAG, "Package " + " has no certificates; ignoring!");
				return null;
			}
		} catch (CertificateEncodingException e) {
			QRomLog.e(TAG, "Exception reading " + sourcePath, e);
			return null;
		} catch (IOException e) {
			QRomLog.e(TAG, "Exception reading " + sourcePath, e);
			return null;
	...
	private static Certificate[] loadCertificates(JarFile jarFile, JarEntry je, byte[] readBuffer) {
		try {
			...
			QRomLog.e(TAG, "Exception reading " + je.getName() + " in " + jarFile.getName(), e);
		} catch (RuntimeException e) {
			QRomLog.e(TAG, "Exception reading " + je.getName() + " in " + jarFile.getName(), e);
		}
		return null;
