...
public class PluginInjector {
    ...
    static void resetActivityContext(final Context pluginContext, final Activity activity, final int pluginAppTheme) {
        ...
        // 注意这里重设回context,是为了解决MEIZU等奇葩机型对系统statusBar的定制问题
    ...
    // break;
    /* package */
    static void replacePluginServiceContext(String serviceName) {
    ...
    /* package */
    static void replacePluginServiceContext(String servieName, Service service) {
    ...
    /* package */
    static void replaceHostServiceContext(String serviceName) {
        Map<IBinder, Service> services = HackActivityThread.get().getServices();
        if (services != null) {
            Iterator<Service> itr = services.values().iterator();
            while (itr.hasNext()) {
