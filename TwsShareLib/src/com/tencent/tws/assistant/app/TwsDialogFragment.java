 ...
 *      embed}
 */
public class TwsDialogFragment extends Fragment
    ...
    public void setStyle(int style, int theme) {
        ...
        if (mStyle == STYLE_NO_FRAME || mStyle == STYLE_NO_INPUT) {
            mTheme = R.style.Theme_tws_Second_Dialog_Alert;
        }
        if (theme != 0) {
            mTheme = theme;
        }
    ...
    public TwsDialog onCreateDialog(Bundle savedInstanceState) {
        return new TwsDialog(getActivity(), getTheme());
    }
	...
	public int getContainerId() {
        try {
            Class<?> clazz1 = ReflectUtils.forClassName("android.app.Fragment");
            int i = (Integer)ReflectUtils.getFieldValue("mContainerId", this, clazz1);
            return i;
        } catch (Exception e) {
            e.printStackTrace();
        }finally{
