...
public final class PreferenceScreen extends PreferenceGroup implements AdapterView.OnItemClickListener,
    ...
    private void showDialog(Bundle state) {
		...
		mListView.setHeaderBlankWithStatusbar(false);
        mListView.setFooterBlank(false);
		int mPadding = (int)context.getResources().getDimension(R.dimen.tws_listview_item_padding_side_left);
		int mMargin = (int)context.getResources().getDimension(R.dimen.preference_fragment_padding_side);
		int size = getPreferenceCount();
		if(size <= 0){
			return;
		}
		Preference mPreference = getPreference(0);
		//setPadding(int left, int top, int right, int bottom)
		if(mPreference instanceof PreferenceCategory){
			mListView.setPadding(mMargin,0,mMargin,0);
		}else{
			int mPaddingTop = (int)context.getResources().getDimension(R.dimen.preference_item_padding_side_top_no_preferencecategory);
			mListView.setPadding(mMargin,0,mMargin,0);
		}
		//mListView.setPadding(mPadding,mPadding,mPadding,0);
//		mListView.setSelector(R.color.transparent);
        ...
        TwsDialog dialog = mDialog = new TwsDialog(context, context.getThemeResId());
        if (TextUtils.isEmpty(title)) {
            dialog.getWindow().requestFeature(Window.FEATURE_NO_TITLE);
        } else {
            dialog.setTitle(title);
        }
		dialog.setShowActionbar(true);
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        final TwsDialog dialog = mDialog;
