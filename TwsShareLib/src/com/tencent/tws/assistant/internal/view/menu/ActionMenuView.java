...
public class ActionMenuView extends LinearLayout implements MenuBuilder.ItemInvoker, MenuView {
    ...
    static final int MIN_CELL_SIZE = 60; // dips
    ...
    static final int OVERFLOW_BUTTON_WIDTH = 44;
    ...
    public ActionMenuView(Context context, AttributeSet attrs) {
        ...
        mOverflowButtonWidth = getResources().getDimensionPixelSize(R.dimen.actionbar_overflow_minwidth);
        if (mOverflowButtonWidth < 1) {
            mOverflowButtonWidth = (int) (OVERFLOW_BUTTON_WIDTH * density);
        }
        ...
        if (mFormatItems) {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if (lp.isOverflowButton) {
                    childWidthSpec = MeasureSpec.makeMeasureSpec(mOverflowButtonWidth, MeasureSpec.EXACTLY);
                }
            ...
            super.onMeasure(MeasureSpec.makeMeasureSpec(menuWidth, MeasureSpec.EXACTLY), heightMeasureSpec);
        }
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (childCount == 1 && !hasOverflow) {
        	...
        	final int height = mContext.getResources().getDimensionPixelSize(R.dimen.tws_actionbar_split_height);
            final View v = getChildAt(0);
            v.measure(0, 0);
            final int width = v.getMeasuredWidth();
            final int midHorizontal = (right - left) / 2;
            final int l = midHorizontal - screenWidth / 2;
            final int t = midVertical - height / 2;
            v.layout((l+screenWidth-t-width)/2, 0, (l+screenWidth+t+width)/2, (t+height));
            return;
        }
        
        if (childCount == 2 
        		&& !((LayoutParams)getChildAt(0).getLayoutParams()).isOverflowButton && ((LayoutParams)getChildAt(1).getLayoutParams()).isOverflowButton 
        		&& ((ViewGroup)getChildAt(0)).getChildAt(0).getMeasuredWidth() == 0) {
        	final int textPadding = mContext.getResources().getDimensionPixelSize(R.dimen.actionbar_mode_padding);
        	View v = getChildAt(0);
        	View textView = ((ViewGroup)v).getChildAt(1);
        	textView.measure(0, 0);
        	int width = textView.getMeasuredWidth();
        	int height = v.getMeasuredHeight();
        	int t = midVertical - height / 2;
        	v.layout(0, 0, textPadding*2+width, height);
        	v.setPadding(0, v.getPaddingTop(), 0, v.getPaddingBottom());
        	textView.layout(textPadding, textView.getTop(), textView.getMeasuredWidth()+textPadding, textView.getBottom());
        	return;
        }
