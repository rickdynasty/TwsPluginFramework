 ...
 */
public class TwsDrawableContainer extends Drawable implements Drawable.Callback {
    private static final boolean DEBUG = false;
    private static final String TAG = "TwsDrawableContainer";
    ...
    public void setTwsTintList(ColorStateList tint) {
        ...
        if (mDrawableContainerState.mTintList != tint) {
            ...
            if (mCurrDrawable != null) {
                updateTint(mCurrDrawable.mutate(), tint, mTwsTintMode);
            }
        }
    }

    public void setTwsTintMode(Mode tintMode) {
        mDrawableContainerState.mHasTintMode = true;

        mTwsTintMode = tintMode;

        if (mDrawableContainerState.mTintMode != tintMode) {
            mDrawableContainerState.mTintMode = tintMode;

            if (mCurrDrawable != null) {
                updateTint(mCurrDrawable.mutate(), mTwsTintList, tintMode);
            }
        }
    }
...
//            mCurrDrawable.mutate().setAutoMirrored(mDrawableContainerState.mAutoMirrored);
    ...
    //tws-start animationStateListDrawable get stateIndex::2014-11-27
    ...
    //tws-end animationStateListDrawable get stateIndex::2014-11-27
    ...
    private void initializeDrawableForDisplay(Drawable d) {
        ...
        if (mDrawableContainerState.mHasColorFilter) {
            ...
            if (mDrawableContainerState.mHasTintList) {
                updateTint(d, mDrawableContainerState.mTintList, mDrawableContainerState.mTintMode);
            }
    ...
    public abstract static class DrawableContainerState extends ConstantState {
        final TwsDrawableContainer mOwner;
        ...
        DrawableContainerState(DrawableContainerState orig, TwsDrawableContainer owner,
                Resources res) {
            mOwner = owner;
...
//                    drawables[i].clearMutated();
    ...
    private boolean updateTint(Drawable d, ColorStateList tint, PorterDuff.Mode tintMode) {
        if (tint != null && tintMode != null) {
            final int color = tint.getColorForState(d.getState(), tint.getDefaultColor());
            final PorterDuff.Mode mode = tintMode;
