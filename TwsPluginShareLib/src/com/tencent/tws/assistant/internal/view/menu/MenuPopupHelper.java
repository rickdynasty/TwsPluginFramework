...
public class MenuPopupHelper implements AdapterView.OnItemClickListener, View.OnKeyListener,
		ViewTreeObserver.OnGlobalLayoutListener, PopupWindow.OnDismissListener, View.OnAttachStateChangeListener,
		MenuPresenter {
	...
	public Bitmap takeMenuBGBlur(int count) {
		try {
			...
			int startY = mPopup.isPopupFromTop ? (res.getDimensionPixelSize(R.dimen.tws_action_bar_height)) : (res
					.getDisplayMetrics().heightPixels
					- res.getDimensionPixelSize(R.dimen.tws_listview_item_height)
					* count - res.getDimensionPixelSize(R.dimen.tws_actionbar_split_height));

			Bitmap bitmap2 = Bitmap.createBitmap(bitmap,
					res.getDisplayMetrics().widthPixels - res.getDimensionPixelSize(R.dimen.tws_popup_menu_min_width)
							+ 50, startY + 50, res.getDimensionPixelSize(R.dimen.tws_popup_menu_min_width) - 100,
	...
	public boolean tryShow(boolean isTransparent, boolean isBlurBG, boolean[] isRedPoint, int[] textColors) {
		mPopup = new ListPopupWindow(mContext, null, R.attr.popupMenuStyle, isTransparent, isBlurBG, isRedPoint,
				textColors);
		...
		int y = location[1];
		int mActionBarHeight = (int) mContext.getResources().getDimension(R.dimen.tws_action_bar_height);
		...
		if (isTransparent) {
			if (isBlurBG) {
				Bitmap blurBGBitmap = takeMenuBGBlur(mMenu.getVisibleItems().size() - mMenu.getActionItems().size());
				...
				mPopup.setBackgroundDrawable(mContext.getResources().getDrawable(
						R.drawable.menu_dropdown_panel_holo_light_trans));
			}
		}
		...
		mPopup.getListView().setDivider(mContext.getResources().getDrawable(R.drawable.list_divider_holo_light));
	...
	private class MenuAdapter extends BaseAdapter {
		...
		public int getCount() {
			ArrayList<MenuItemImpl> items = mOverflowOnly ? mAdapterMenu.twsGetNonActionItems() : mAdapterMenu
					.twsGetVisibleItems();
			if (mExpandedIndex < 0) {
				return items.size();
			}
	...
	/* tws-start::add 2012-12-03 */
	private void twsSetWidth() {
		int minWidth = mContext.getResources().getDimensionPixelSize(R.dimen.tws_popup_menu_min_width);
