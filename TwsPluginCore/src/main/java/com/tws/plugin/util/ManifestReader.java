...
/**
 * @author Dmitry Skiba Binary xml files parser. Parser has only two states: (1)
 *         Operational state, which parser obtains after first successful call
 *         to next() and retains until open(), close(), or failed call to
 *         next(). (2) Closed state, which parser obtains after open(), close(),
 *         or failed call to next(). In this state methods return invalid values
 *         or throw exceptions.
 */
 ...
 * operate with int, use -1 as 'invalid value'. !! functions expect
 * 'prefix'+'uri' pairs, not 'uri'+'prefix' !!
 */
