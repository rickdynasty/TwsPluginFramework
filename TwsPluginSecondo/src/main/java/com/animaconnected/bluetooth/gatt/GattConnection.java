...
class GattConnection {
    ...
    private void doConnect() {
        if (mShouldBeDisconnected) {
            Log.d(TAG, "Not connecting to " + mDevice.getAddress() + " due to cancellation");
        } else {
            ...
            if (BluetoothAdapter.getDefaultAdapter().isEnabled()) {
                ...
                Log.d(TAG, "Bluetooth is not enabled, just wait for it to be enabled!");
            }
        }
    }

    private void startActiveConnect() {
        Log.d(TAG, "Connecting to " + mDevice.getAddress());
    ...
    public void disconnect() {
        if (!mShouldBeDisconnected) {
            if (mIsDebugEnabled) Log.d(TAG, "Disconnect");
            ...
            if (mExecutingCommand == null) {
                ...
                if (mIsDebugEnabled) Log.d(TAG, "Waiting for current command to finish");
        ...
        if (!mIsDisconnected) {
            ...
            if (mGatt != null) {
                ...
                try {
                    ...
                    Log.d(TAG, "BluetoothGatt.close() threw: " + error);
    ...
    private void onDeviceWasBonded() {
        ...
        mListener.onBonded();
        if (mIsBonding) {
            Log.d(TAG, "Bonding finished. Discovering services.");
            mIsBonding = false;
            discoverServices();
        } else if (mExecutingCommand != null && mGatt != null) {
            Log.d(TAG, "Bonded! Re-running current command.");
    ...
    private void onBondingTimeout() {
        if (mIsBonding) {
            Log.d(TAG, "Bonding timed out. Discovering services.");
            mIsBonding = false;
            discoverServices();
        }
    }
    ...
    private void onConnectingTimeout() {
        Log.d(TAG, "Connection took too long, terminate and retry the whole connection!.");
    ...
    private void onBluetoothTurnedOn() {
        Log.d(TAG, "Bluetooth was turned on. Lets retry the whole connection!");
        close();
    }
    ...
    public void read(final UUID service,
        ...
        if (mIsDebugEnabled) Log.d(TAG, "read: " + service + " / " + characteristic);
    ...
    public void write(final UUID service,
        ...
        if (mIsDebugEnabled) Log.d(TAG, "write: " + service + " / " + characteristic);
        final BluetoothGattCharacteristic c = getCharacteristic(service, characteristic);
        if (c != null) {
    ...
    public void setUseRefreshService(final boolean enable) {
        ...
        Log.d(TAG, enable ? "Using refreshServices" : "Using removeBond");
    }

    void setNotification(final UUID service,
                         final UUID characteristic,
                         final Callback<Void> callback) {
        Log.d(TAG, "Set notification: " + service + " / " + characteristic);
        final BluetoothGattCharacteristic c = getCharacteristic(service, characteristic);
        if (c != null) {
    ...
    private void runCommand(final GattCommand command) {
        if (mShouldBeDisconnected) {
            if (mIsDebugEnabled) Log.d(TAG, "Rejecting new command since we're disconnecting");
            command.onError(new RuntimeException("Disconnecting"));
        } else if (mExecutingCommand == null) {
            if (mIsDebugEnabled) Log.d(TAG, "Starting command directly: " +
                    command.getClass().getSimpleName());
            mExecutingCommand = command;
            startTimeout(COMMAND_TIMEOUT);
            command.execute(mGatt);
        } else {
            if (mIsDebugEnabled) Log.d(TAG, "Queuing command");
    ...
    private void onCharacteristicRead(final byte[] value, final int status) {
        ...
        if (mIsDebugEnabled) Log.d(TAG, "Characteristic read (status = " + status + ")");
    ...
    private void onCharacteristicWrite(final int status) {
        if (mExecutingCommand == null) return;
        if (mIsDebugEnabled) Log.d(TAG, "Characteristic written (status = " + status + ")");
    ...
    private void onDescriptorWrite(final int status) {
        if (mExecutingCommand == null) return;
        if (mIsDebugEnabled) Log.d(TAG, "Descriptor written (status = " + status + ")");
    ...
    private void retryCommand() {
        if (mShouldBeDisconnected) {
            ...
            Log.d(TAG, "Retrying command...");
            mExecutingCommand.execute(mGatt);
        }
    }

    private void onSuccessfulCommand() {
        if (mIsDebugEnabled) Log.d(TAG, "Command succeeded");
        stopTimeout();
        ...
        if (mExecutingCommand != null) { // queue not empty
            if (mIsDebugEnabled) Log.d(TAG, "Executing queued command: " +
                    mExecutingCommand.getClass().getSimpleName());
            startTimeout(COMMAND_TIMEOUT);
            mExecutingCommand.execute(mGatt);
        } else {
            Log.d(TAG, "No command in queue");
        }
    }
    ...
    private void onCharacteristicChanged(final UUID service,
        ...
        if (mIsDebugEnabled) Log.d(TAG, "Characteristic changed: " + characteristic);
    ...
    private void onConnectionStateChange(final int newState, final int status) {
        ...
        if (newState == BluetoothProfile.STATE_CONNECTED) {
            if (mCurrentConnectionPhase == GattConnectionAttempt.CONNECTING_ACTIVE) {
                ...
                Log.d(TAG, "Weird connectionPhaseState, should not happen: " +
            ...
            if (mIsBonding && !mShouldBeDisconnected && mGatt != null && !mSkipBonding) {
                Log.d(TAG, "Bonding seemed to trigger a disconnect. Retry without bond request.");
                ...
                if (status != BluetoothGatt.GATT_SUCCESS) {
                    Log.d(TAG, "Disconnected with an error code. (Don't) Remove bond here.");
                }
                close();
                ...
                Log.d(TAG, "Got directly disconnected with an error code. Removing bond and try again.");
                ...
                if (mGattConnectionInstructor.shallContinueTrying(mCurrentConnectionPhase)) {
                    Log.d(TAG, "Starting a passive connection to " + mDevice.getAddress());
                    startPassiveConnect();
                } else {
                    close();
                }
    ...
    private void startConnectGattTimeout(final long timeoutTime) {
        Log.d(TAG, "Starting connection timeout with timeoutTime: " + timeoutTime + " ...");
    ...
    private void stopConnectGattTimeout() {
        Log.d(TAG, "Stopping connection timeout...");
        mHandler.removeCallbacks(mConnectTooLongRunnable);
    }
    ...
    private void createBondIfNeeded() {
        if (mDevice.getBondState() == BluetoothDevice.BOND_BONDED) {
            ...
            Log.d(TAG, "Skipping bonding...");
        } else if (mDevice.getBondState() == BluetoothDevice.BOND_BONDING) {
            Log.d(TAG, "Device is bonding");
            mIsBonding = true;
        } else {
            Log.d(TAG, "Requesting bonding");
    ...
    private void startBondingTimeout() {
        Log.d(TAG, "Starting bonding timeout...");
    ...
    private void stopBondingTimeout() {
        Log.d(TAG, "Stopping bonding timeout...");
        mHandler.removeCallbacks(mBondingRunnable);
    }
    ...
    private void startTimeout(final long timeoutMs) {
        if (mIsDebugEnabled) Log.d(TAG, "Starting timeout");
        mHandler.postDelayed(mTimeoutRunnable, timeoutMs);
    }

    private void stopTimeout() {
        if (mIsDebugEnabled) Log.d(TAG, "Canceling timeout");
