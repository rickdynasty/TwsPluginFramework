 ...
 * {@link Activity#showDialog(int)}, and {@link Activity#dismissDialog(int)}. If these methods are used,
 * {@link #getOwnerActivity()} will return the Activity that managed this dialog.
 * 
 * <p>
 * Often you will want to have a Dialog display on top of the current input method, because there is no reason
 * for it to accept text. You can do this by setting the
 * {@link WindowManager.LayoutParams#FLAG_ALT_FOCUSABLE_IM WindowManager.LayoutParams.FLAG_ALT_FOCUSABLE_IM}
 ...
 * </div>
 */
public class TwsDialog implements DialogInterface, Window.Callback, KeyEvent.Callback,
     ...
     * 
     * @param context
     *            The Context the Dialog is to run it. In particular, it uses the window manager and theme in
     *            this context to present its UI.
     */
     ...
     * 
     * @param context
     *            The Context in which the Dialog should run. In particular, it uses the window manager and
     *            theme from this context to present its UI.
     * @param theme
     *            A style resource describing the theme to use for the window. See <a href="{@docRoot}
     *            guide/topics/resources/available-resources.html#stylesandthemes">Style and Theme
     *            Resources</a> for more information about defining and using styles. This theme is applied on
     *            top of the current theme in <var>context</var>. If 0, the default dialog theme will be used.
     */
    ...
    /**
     * Sets the Activity that owns this dialog. An example use: This Dialog will use the suggested volume
     * control stream of the Activity.
     * 
     * @param activity
    ...
    /**
     * Returns the Activity that owns this Dialog. For example, if {@link Activity#showDialog(int)} is used to
     * show this Dialog, that Activity will be the owner (by default). Depending on how this dialog was
     ...
     * Note that you should not override this method to do initialization when the dialog is shown, instead
     * implement that in {@link #onStart}.
     */
    public void show() {
        if (mShowing) {
            if (mDecor != null) {
    ...
    /**
     * Similar to {@link Activity#onCreate}, you should initialize your dialog in this method, including
     * calling {@link #setContentView}.
     * 
     * @param savedInstanceState
     *            If this dialog is being reinitalized after a the hosting activity was previously shut down,
     *            holds the result from the most recent call to {@link #onSaveInstanceState}, or null if this
     *            is the first time.
     */
     ...
     * 
     * The default implementation saves the state of its view hierarchy, so you'll likely want to call through
     * to super if you override this to save additional state.
     * 
     ...
     * 
     * The default implementation restores the state of the dialog's view hierarchy that was saved in the
     * default implementation of {@link #onSaveInstanceState()}, so be sure to call through to super when
    ...
    /**
     * Retrieve the current Window for the activity. This can be used to directly access parts of the Window
     * API that are not available through Activity/Screen.
     * 
    ...
    /**
     * Call {@link android.view.Window#getCurrentFocus} on the Window if this Activity to return the currently
     * focused view.
     * 
    ...
    /**
     * Set the screen content from a layout resource. The resource will be inflated, adding all top-level
     * views to the screen.
     * 
     * @param layoutResID
    ...
    public void setContentView(int layoutResID) {
    	...
    	FrameLayout.LayoutParams vlp = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout.LayoutParams.WRAP_CONTENT);
    	vlp.gravity = Gravity.CENTER;
        mWindow.setContentView(mDialogView, vlp);
    }

    /**
     * Set the screen content to an explicit view. This view is placed directly into the screen's view
     * hierarchy. It can itself be a complex view hierarhcy.
     * 
     * @param view
    ...
    public void setContentView(View view) {
		...
		if(mShowActionbar){
			if(mContentParent==null)
			{
				...
				mActionBarView.setContentHeight((int)mContext.getResources().getDimension(R.dimen.tws_action_bar_height));
			...
			mContentParent.addView(view, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
			((ViewGroup)mDecor).removeAllViews();
			((ViewGroup)mDecor).addView(mDialogView, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
    ...
    /**
     * Add an additional content view to the screen. Added after any existing ones in the screen -- existing
     * views are NOT removed.
     * 
     * @param view
     *            The desired content to display.
     * @param params
    ...
    /**
     * Set the title text for this dialog's window. The text is retrieved from the resources with the supplied
     * identifier.
     * 
     * @param titleId
    ...
    /**
     * Called when the dialog has detected the user's press of the back key. The default implementation simply
     * cancels the dialog (only if it is cancelable), but you can override this to do whatever you want.
     */
    ...
    /**
     * Called when an key shortcut event is not handled by any of the views in the Dialog. Override this
     * method to implement global key shortcuts for the Dialog. Key shortcuts can also be implemented by
     * setting the {@link MenuItem#setShortcut(char, char) shortcut} property of menu items.
     * 
     * @param keyCode
    ...
    /**
     * Called when a touch screen event was not handled by any of the views under it. This is most useful to
     * process touch events that happen outside of your window bounds, where there is no view to receive it.
     * 
     * @param event
     *            The touch screen event being processed.
     * @return Return true if you have consumed the event, false if you haven't. The default implementation
     *         will cancel the dialog when a touch happens outside of the window bounds.
     */
    ...
    /**
     * Called when the trackball was moved and not handled by any of the views inside of the activity. So, for
     * example, if the trackball moves while focus is on a button, you will receive a call here because
     * buttons do not normally do anything with trackball events. The call here happens <em>before</em>
     * trackball movements are converted to DPAD key events, which then get sent back to the view hierarchy,
     * and will be processed at the point for things like focus navigation.
     * 
     * @param event
     ...
     * <p>
     * Generic motion events describe joystick movements, mouse hovers, track pad touches, scroll wheel
     * movements and other input events. The {@link MotionEvent#getSource() source} of the motion event
     * specifies the class of input that was received. Implementations of this method must examine the bits in
     * the source before processing the event. The following code example shows how this is done.
     * </p>
     * <p>
     * Generic motion events with source class {@link android.view.InputDevice#SOURCE_CLASS_POINTER} are
     * delivered to the view under the pointer. All other generic motion events are delivered to the focused
     * view.
     * </p>
     * <p>
    ...
    /**
     * Called to process key events. You can override this to intercept all key events before they are
     * dispatched to the window. Be sure to call this implementation for key events that should be handled
     * normally.
     * 
     * @param event
     *            The key event.
    ...
    /**
     * Called to process a key shortcut event. You can override this to intercept all key shortcut events
     * before they are dispatched to the window. Be sure to call this implementation for key shortcut events
    ...
    /**
     * Called to process touch screen events. You can override this to intercept all touch screen events
     * before they are dispatched to the window. Be sure to call this implementation for touch screen events
     * that should be handled normally.
     * 
     * @param ev
    ...
    /**
     * Called to process trackball events. You can override this to intercept all trackball events before they
     * are dispatched to the window. Be sure to call this implementation for trackball events that should be
     * handled normally.
     * 
     * @param ev
     *            The trackball event.
    ...
    /**
     * Called to process generic motion events. You can override this to intercept all generic motion events
     * before they are dispatched to the window. Be sure to call this implementation for generic motion events
     * that should be handled normally.
     * 
     * @param ev
     ...
     * {@link Activity#onCreateOptionsMenu(Menu)} if the client desires the same menu for this Dialog.
     * 
    ...
    /**
     * It is usually safe to proxy this call to the owner activity's
     * {@link Activity#onPrepareOptionsMenu(Menu)} if the client desires the same menu for this Dialog.
     * 
     ...
     * {@inheritDoc}
     * 
     * Note that if you override this method you should always call through to the superclass implementation
     * by calling super.onActionModeStarted(mode).
     */
     ...
     * 
     * Note that if you override this method you should always call through to the superclass implementation
     * by calling super.onActionModeFinished(mode).
     */
    ...
    /**
     * Sets whether this dialog is canceled when touched outside the window's bounds. If setting to true, the
     * dialog is set to be cancelable if not already set.
     * 
     * @param cancel
    ...
    /**
     * Cancel the dialog. This is essentially the same as calling {@link #dismiss()}, but it will also call
     * your {@link DialogInterface.OnCancelListener} (if registered).
     */
    public void cancel() {
     ...
     * <p>
     * This will only be invoked when the dialog is canceled, if the creator needs to know when it is
