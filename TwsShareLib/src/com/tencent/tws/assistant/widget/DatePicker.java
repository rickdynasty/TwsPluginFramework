...
public class DatePicker extends FrameLayout {
    ...
    private static final int DEFAULT_END_YEAR = 2016;
...
//        mCurrentDate = (ChineseCalendar) getCalendarForLocale(mCurrentDate, locale);
...
//        return Character.isDigit(mShortMonths[Calendar.JANUARY].charAt(0));
    ...
    private void updateSpinners() {
        ...
        mYearSpinner.setWrapSelectorWheel(false);//不循环
        mYearSpinner.setFormatter(mYearFormatter);
        if (mIsLunar) {
            ...
            if (mCurrentDate.get(ChineseCalendar.CHINESE_YEAR) == mMinDate.get(ChineseCalendar.CHINESE_YEAR)) {
                ...
                if (mCurrentDate.get(ChineseCalendar.CHINESE_MONTH) == mMinDate
                        .get(ChineseCalendar.CHINESE_MONTH)) {
            ...
            } else if (mCurrentDate.get(ChineseCalendar.CHINESE_YEAR) == mMaxDate
                    .get(ChineseCalendar.CHINESE_YEAR)) {
                mMonthSpinner.setMinValue(0);
                mMonthSpinner.setMaxValue(mMaxDate.get(ChineseCalendar.CHINESE_MONTH));
                mMonthSpinner.setWrapSelectorWheel(false);
                if (mCurrentDate.get(ChineseCalendar.CHINESE_MONTH) == mMaxDate
                        .get(ChineseCalendar.CHINESE_MONTH)) {
                    mDaySpinner.setMinValue(1);
            ...
            int leapMonth = ChineseCalendar.getChineseLeapMonth(mCurrentDate
                    .get(ChineseCalendar.CHINESE_YEAR));
...
//        Log.d(LOG_TAG,
//                "setIsLunar=" + isLunar + ",lunarYear=" + mCurrentDate.get(ChineseCalendar.CHINESE_YEAR)
