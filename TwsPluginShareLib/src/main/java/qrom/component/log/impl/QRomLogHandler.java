...
/**
 * Title: QRomLogHandler
 * Package: qrom.component.log.impl
 * Author: interzhang
 * Date: 14-3-17 下午3:58
 * Version: v1.0
 */
public class QRomLogHandler extends Handler {
    ...
    public QRomLogHandler(Looper looper, QRomLogBaseConfig config) {
        super(looper);
        mConfig = config;
    ...
    private String getLogFileSubName(boolean isTrace) {
        if (mProcessName == null) {
            mProcessName = QRomLogUtils.getProcessName();
    ...
    public ArrayList<File> prepareUploadTraceLog() {
        ...
        File dir1 = QRomLogUtils.getLogFileDirectory(mConfig.getPackageName(), true);
        File dir2 = QRomLogUtils.getLogFileDirectory(mConfig.getPackageName(), false);
    ...
    private void resetCurFileLog() {
        ...
        if (mLogFile != null) {
            ...
            File dir = QRomLogUtils.getLogFileDirectory(mConfig.getPackageName(), false);
        ...
        switch (msg.what) {
            case LOG_MSG_LOG_WRITE:  // debug的文件日志
                ...
                try {
                    if (!mLogWriterFlag || mLogFileDate != mCalendar.get(Calendar.DAY_OF_MONTH)) {
                        File dir = QRomLogUtils.getLogFileDirectory(mConfig.getPackageName(), false);
                        if (dir != null) {
                            mLogFile = QRomLogUtils.createNewFile(dir.getAbsolutePath(), "log_" + getLogFileSubName(false) + ".log");
                	...
                	if	(ex.getMessage() != null)
                		QRomLog.w("TraceLog", ex.getMessage());
                	else
            ...
            case LOG_MSG_TRACE_WRITE:  // 用户行为的trace日志
                ...
                if (!mTraceWriterFlag || mTraceFileDate != mCalendar.get(Calendar.DAY_OF_MONTH)) {
                    File dir = QRomLogUtils.getLogFileDirectory(mConfig.getPackageName(), false);
                    if (dir != null) {
                        mTraceFile = QRomLogUtils.createNewFile(dir.getAbsolutePath(), "trace_" + getLogFileSubName(true) + ".txt");
                        if (mTraceFile != null) {
            ...
            case LOG_MSG_FILE_CLEAN:  // 清理过期日志
                delExpiredLogFile(QRomLogUtils.getLogFileDirectory(mConfig.getPackageName(), true), 3 * LOG_DAYMILLISECOND);
                delExpiredLogFile(QRomLogUtils.getLogFileDirectory(mConfig.getPackageName(), false), 3 * LOG_DAYMILLISECOND);
                delExpiredLogFile(QRomLogUtils.getCrashFileDirectory(mConfig.getPackageName()), 3 * LOG_DAYMILLISECOND);
                break;
