 ...
 */
public abstract class PreferenceActivity extends TwsListActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*TypedArray sa = obtainStyledAttributes(null,
                ...
                R.attr.preferenceActivityStyle,
                0);
        ...
        mPreferenceHeaderItemResId = sa.getResourceId(
                ...
                R.layout.preference_header_item);
        mPreferenceHeaderRemoveEmptyIcon = sa.getBoolean(
                R.styleable.PreferenceActivity_headerRemoveIconIfEmpty,
                false);

        sa.recycle();*/
        final int layoutResId = R.layout.preference_list_content;
        mPreferenceHeaderItemResId = R.layout.preference_header_item;
            ...
            while ((type=parser.next()) != XmlPullParser.END_DOCUMENT
                ...
                if ("header".equals(nodeName)) {
                    ...
                    TypedArray sa = getResources().obtainAttributes(attrs,
                            android.R.styleable.PreferenceHeader);
                    header.id = sa.getResourceId(
                            android.R.styleable.PreferenceHeader_id,
                            (int)HEADER_ID_UNDEFINED);
                    TypedValue tv = sa.peekValue(
                            android.R.styleable.PreferenceHeader_title);
                    ...
                    tv = sa.peekValue(
                            android.R.styleable.PreferenceHeader_summary);
                    if (tv != null && tv.type == TypedValue.TYPE_STRING) {
                        if (tv.resourceId != 0) {
                    ...
                    tv = sa.peekValue(
                            android.R.styleable.PreferenceHeader_breadCrumbTitle);
                    if (tv != null && tv.type == TypedValue.TYPE_STRING) {
                        if (tv.resourceId != 0) {
                    ...
                    tv = sa.peekValue(
                            android.R.styleable.PreferenceHeader_breadCrumbShortTitle);
                    if (tv != null && tv.type == TypedValue.TYPE_STRING) {
                        if (tv.resourceId != 0) {
                    ...
                    header.iconRes = sa.getResourceId(
                            android.R.styleable.PreferenceHeader_icon, 0);
                    header.fragment = sa.getString(
                            android.R.styleable.PreferenceHeader_fragment);
                    sa.recycle();
    ...
    protected boolean isValidFragment(String fragmentName) {
        if (getApplicationInfo().targetSdkVersion  > android.os.Build.VERSION_CODES.JELLY_BEAN_MR2) {
            throw new RuntimeException(
    ...
    protected void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (mAdapter != null) {
            ...
            if (item instanceof Header) onHeaderClick((Header) item, pos);
        }
    }
    ...
    public void bindPreferences() {
        ...
        if (preferenceScreen != null) {
			...
			Preference mPreference = preferenceScreen.getPreference(0);
			//setPadding(int left, int top, int right, int bottom)
			int mPadding = (int)getResources().getDimension(R.dimen.tws_listview_item_padding_side_left);
			int mMargin = (int)getResources().getDimension(R.dimen.preference_fragment_padding_side);
			if(mPreference instanceof PreferenceCategory){
				mListView.setPadding(mMargin,0,mMargin,0);
//				Log.d("PreferenceActivity","bindPreferences");
			}else{
				int mPaddingTop = (int)getResources().getDimension(R.dimen.preference_item_padding_side_top_no_preferencecategory);
//				Log.d("PreferenceActivity","mPaddingTop ="+mPaddingTop);
				mListView.setPadding(mMargin,0,mMargin,0);
			}
