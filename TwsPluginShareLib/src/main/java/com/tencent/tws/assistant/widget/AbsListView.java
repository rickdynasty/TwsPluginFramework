...
public abstract class AbsListView extends AdapterView<ListAdapter> implements TextWatcher,
        ...
        RemoteViewsAdapterWrapper.RemoteAdapterConnectionCallback {
    ...
    //tws-start add listview bounce feature::2014-08-12
	static final int TOUCH_MODE_RESCROLL = 7;
	//tws-end add listview bounce feature::2014-08-12
     ...
     */
    private RemoteViewsAdapterWrapper mRemoteAdapter;
    ...
    private int mTouchSlopX, mTouchSlopY;
    ...
    private Rect mLastSelectorRect = new Rect();
	...
	private boolean isSetItemSelector;
	private boolean isNeedBounce = true;
	private boolean mClickDelay = true;
	protected boolean mBlurRecycleFlag;
	protected boolean mIsTouchOffsetFlag;
	//tws-end add listview bounce feature::2014-08-12
    ...
    public AbsListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        Drawable dd = a.getDrawable(R.styleable.AbsListView_topListSelector);
        if (dd != null) {
            setTopSelector(dd);
        }

        Drawable ddd = a.getDrawable(R.styleable.AbsListView_bottomListSelector);
        if (ddd != null) {
...
//                R.drawable.list_selector_background));
        ...
        if(bRipple){
			TwsRippleDrawable drawable = new TwsRippleDrawable(
                getResources().getColorStateList(R.color.list_item_ripple_light), 
...
//        return (hasFocus() && !isInTouchMode()) || touchModeDrawsInPressedState();
    ...
    private void drawSelector(Canvas canvas) {
        if (!mSelectorRect.isEmpty()) {
            ...
            if(mTopSelector != null && mSelectorRect.top <= getPaddingTop()) {
                ...
                mSelectorRect.bottom >= getHeight() - getPaddingBottom()) {
                mCurrentSelector = mBottomSelector;
            } else {
                mCurrentSelector = mSelector;
            }
    ...
    void keyPressed() {
        ...
        Drawable selector = mCurrentSelector;
    ...
    public boolean verifyDrawable(Drawable dr) {
        return mSelector == dr || mTopSelector == dr || 
            mBottomSelector == dr || super.verifyDrawable(dr);
    }
    ...
    private boolean startScrollIfNeeded(int x, int y, MotionEvent vtev) {
        ...
        if (overscroll || distance > mTouchSlopY) {
            ...
            if (overscroll) {
                ...
                mMotionCorrection = deltaY > 0 ? mTouchSlopY : -mTouchSlopY;
            }
...
//                    Debug.startMethodTracing("AbsListViewScroll");
            ...
            if (y != mLastY) {
                ...
                if ((mGroupFlags & FLAG_DISALLOW_INTERCEPT) == 0 &&
                        Math.abs(rawDeltaY) > mTouchSlopY) {
                    final ViewParent parent = getParent();
                    if (parent != null) {
                        parent.requestDisallowInterceptTouchEvent(true);
                    }
...
//                        }
    ...
    private boolean startScrollIfNeeded(int x, int y) {
        ...
        if (distanceX > mTouchSlopX || distanceY > mTouchSlopY) {
            createScrollingCache();
            mTouchMode = TOUCH_MODE_SCROLL;
            mMotionCorrection = deltaY > 0 ? mTouchSlopY : -mTouchSlopY;
            // tws-start fix long press bug::2014-08-30
            if (mPendingCheckForLongPress != null)
                removeCallbacks(mPendingCheckForLongPress);
            // tws-end fix long press bug::2014-08-30
            setPressed(false);
            if (getChildCount() > 0) {
                final View motionView = getChildAt(mMotionPosition - mFirstPosition);
                if (motionView != null) {
    ...
    private void scrollIfNeeded(int x, int y) {
        ...
        if (mTouchMode == TOUCH_MODE_SCROLL) {
            if (y != mLastY) {
                if (Math.abs(rawDeltaY) > mTouchSlopY) {
                    final ViewParent parent = getParent();
                    if (parent != null) {
                        parent.requestDisallowInterceptTouchEvent(true);
                    }
    ...
    public void onTouchModeChanged(boolean isInTouchMode) {
        if (isInTouchMode) {
            ...
            if (!isNeedBounce() && (touchMode == TOUCH_MODE_OVERSCROLL || touchMode == TOUCH_MODE_OVERFLING)) {
                if (mFlingRunnable != null) {
                    mFlingRunnable.endFling();
                }
                if (mPositionScroller != null) {
    ...
    private void onTouchDown(MotionEvent ev) {
        ...
        if (!isNeedBounce() && mTouchMode == TOUCH_MODE_OVERFLING) {
            ...
            if (!mDataChanged) {
                if (mTouchMode == TOUCH_MODE_FLING || (isNeedBounce() && mTouchMode == TOUCH_MODE_RESCROLL)) {
    ...
    private void onTouchMove(MotionEvent ev, MotionEvent vtev) {
        ...
        switch (mTouchMode) {
            ...
            case TOUCH_MODE_DONE_WAITING:
                ...
                if (!pointInView(x, y, mTouchSlopY)) {
                    setPressed(false);
                    final View motionView = getChildAt(mMotionPosition - mFirstPosition);
                    if (motionView != null) {
                        motionView.setPressed(false);
    ...
    private void onTouchUp(MotionEvent ev) {
        switch (mTouchMode) {
                ...
                if (child != null) {
                    ...
                    if (inList && !child.hasFocusable()) {
                        ...
                        if (mTouchMode == TOUCH_MODE_DOWN || mTouchMode == TOUCH_MODE_TAP) {
                            if (mPendingCheckForTap != null && mPendingCheckForLongPress != null) {
                            ...
                            if (!mDataChanged && mAdapter.isEnabled(motionPosition)) {
                                ...
                                if (mSelector != null) {
                                    ...
                                    twsSetHotspot(mSelector, x, ev.getY());
                                }
                                if (mTouchModeReset != null) {
                                    removeCallbacks(mTouchModeReset);
                                }
                                mTouchModeReset = new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        if (!mDataChanged && mIsAttached) {
                                            // tws-start click delay::2014-10-30
                                            boolean bRipple = ThemeUtils.isShowRipple(mContext);
                                            if (bRipple) {
                                                postDelayed(performClick, mClickDelay ? 120 : 0);
                                            } else {
                                                performClick.run();
                                            }
                                            // tws-end click delay::2014-10-30
                                        }
                                    }
                                };
                            ...
                            boolean bRipple = ThemeUtils.isShowRipple(mContext);
                            if (bRipple) {
                                postDelayed(performClick, mClickDelay ? 125 : 0);
                            } else {
                                performClick.run();
                            }
                            // tws-end click delay::2014-10-30
            ...
            case TOUCH_MODE_REST:
                ...
                if (childCount > 0) {
                    ...
                    if (!isNeedBounce() && mFirstPosition == 0 && firstChildTop >= contentTop
        ...
        if (mEdgeGlowTop != null && !isNeedBounce()) {
            mEdgeGlowTop.onRelease();
            mEdgeGlowBottom.onRelease();
        }
...
//                Debug.stopMethodTracing();
    ...
    private void onTouchCancel() {
        ...
        switch (mTouchMode) {
            case TOUCH_MODE_SCROLL:
            	...
            	if (getChildAt(0)!= null && getChildAt(0).getTop() < 0 && mFlingRunnable != null)
            		mFlingRunnable.startSpringback();
                break;
            ...
            default:
                mTouchMode = TOUCH_MODE_REST;
                setPressed(false);
                final View motionView = getChildCount() == 0 ? null : this.getChildAt(mMotionPosition
                        - mFirstPosition);
                if (motionView != null) {
                    motionView.setPressed(false);
                }
    ...
    protected void onOverScrolled(int scrollX, int scrollY, boolean clampedX, boolean clampedY) {
        if (mScrollY != scrollY && !isNeedBounce()) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_DOWN: {
            ...
            mVelocityTracker.addMovement(ev);
            if (touchMode == TOUCH_MODE_FLING || (touchMode == TOUCH_MODE_RESCROLL && isNeedBounce())) {
                return true;
            }
            break;
        }
    ...
    private class FlingRunnable implements Runnable {
        ...
        FlingRunnable() {
            mScroller = new OverScroller(getContext(), new CubicEaseInInterpolator());
        }
...
//            mScroller.setInterpolator(linear ? sLinearInterpolator : null);
        ...
        void endFling() {
            ...
            if (!isNeedBounce() && mCheckFlywheel != null) {
            ...
            if (isNeedBounce() && oldTouchMode == TOUCH_MODE_RESCROLL) {
        ...
        boolean scrollToAdjustViewsUpOrDown() {
            ...
            final Rect listPadding = mListPadding;
            final int topOffset = firstTop - (listPadding.top + mFirstTopOffset);
            ...
            if (cannotScrollDown) {
                ...
                startScroll(-duration, (int) Math.abs(duration / mDensity) + 450, true);
            ...
            case TOUCH_MODE_FLING: {
                ...
                if (isNeedBounce()) {
                    ...
                    if (childCount == getCount()) {
                        int totalHeight = 0;
                        totalHeight = getDisplayItemTotalHeight(childCount);
...
//                                Debug.stopMethodTracing();
    ...
    boolean trackMotionScroll(int deltaY, int incrementalDeltaY) {
        ...
        if (isNeedBounce()) {
            return twsTrackMotionScroll(deltaY, incrementalDeltaY, childCount);
        } else {
            final int firstTop = getChildAt(0).getTop();
            final int lastBottom = getChildAt(childCount - 1).getBottom();
...
//                        }
    ...
    public void setRemoteViewsAdapter(Intent intent) {
        ...
        mRemoteAdapter = new RemoteViewsAdapterWrapper(getContext(), intent, this);
...
//                    AccessibilityEvent.CONTENT_CHANGE_TYPE_SUBTREE);
            ...
            if (android.os.Build.VERSION.SDK_INT > 15) {
            	scrapHasTransientState = scrap.hasTransientState();
            }
            else {
    ...
    private boolean getClipToPadding() {
        //align to viewgroup
        final int FLAG_CLIP_TO_PADDING = 0x2;
    ...
    public void setSelector(int resID, boolean shaped) {
        mDrawShapedSelector = shaped;

        setSelector(resID);
    }

    public void setSelector(Drawable sel, boolean shaped) {
        mDrawShapedSelector = shaped;
    ...
    public void setTopSelector(Drawable sel) {
        mTopSelector = sel;
        mTopSelector.setCallback(this);
    }

    public void setBottomSelector(int resId) {
        setBottomSelector(getResources().getDrawable(resId));
    }

    public void setBottomSelector(Drawable sel) {
        mBottomSelector = sel;
        mBottomSelector.setCallback(this);
    }

    public void setSelector(int selResId, int topSelResId, int bottomSelResId) {
        setSelector(selResId);
        setTopSelector(topSelResId);
        setBottomSelector(bottomSelResId);
    }

    public void setSelector(Drawable sel, Drawable topSel, Drawable bottomSel) {
        setSelector(sel);
    ...
    String state2String(int state) {
        ...
        switch(state) {
            case android.R.attr.state_above_anchor:
                return "state_above_anchor" + ((fValue ? "(false)" : ""));
            case android.R.attr.state_accelerated:
                return "state_accelerated" + (fValue ? "(false)" : "");
            case android.R.attr.state_activated:
                return "state_activated" + (fValue ? "(false)" : "");
            case android.R.attr.state_active:
                return "state_active" + (fValue ? "(false)" : "");
            case android.R.attr.state_checkable:
                return "state_checkable" + (fValue ? "(false)" : "");
            case android.R.attr.state_checked:
                return "state_checked" + (fValue ? "(false)" : "");
            case android.R.attr.state_drag_can_accept:
                return "state_drag_can_accept" + (fValue ? "(false)" : "");
            case android.R.attr.state_drag_hovered:
                return "state_drag_hovered" + (fValue ? "(false)" : "");
            case android.R.attr.state_empty:
                return "state_empty" + (fValue ? "(false)" : "");
            case android.R.attr.state_enabled:
                return "state_enabled" + (fValue ? "(false)" : "");
            case android.R.attr.state_expanded:
                return "state_expanded" + (fValue ? "(false)" : "");
            case android.R.attr.state_first:
                return "state_first" + (fValue ? "(false)" : "");
            case android.R.attr.state_focused:
                return "state_focused" + (fValue ? "(false)" : "");
            case android.R.attr.state_hovered:
                return "state_hovered" + (fValue ? "(false)" : "");
            case android.R.attr.state_last:
                return "state_last" + (fValue ? "(false)" : "");
            case android.R.attr.state_long_pressable:
                return "state_long_pressable" + (fValue ? "(false)" : "");
            case android.R.attr.state_middle:
                return "state_middle" + (fValue ? "(false)" : "");
            case android.R.attr.state_multiline:
                return "state_multiline" + (fValue ? "(false)" : "");
            case android.R.attr.state_pressed:
                return "state_pressed" + (fValue ? "(false)" : "");
            case android.R.attr.state_selected:
                return "state_selected" + (fValue ? "(false)" : "");
            case android.R.attr.state_single:
                return "state_single" + (fValue ? "(false)" : "");
            case android.R.attr.state_window_focused:
                return "state_window_focused" + (fValue ? "(false)" : "");
        }

//        android.util.Log.d("Hendy", "unknown state: 0x" + Integer.toHexString(state));

        return null;
    }

	//tws-start add bounce switch getter/setter::2014-09-02
	public boolean isNeedBounce() {
		return isNeedBounce;
	}

	public void setIsNeedBounce(boolean isNeedBounce) {
		this.isNeedBounce = isNeedBounce;
	}
	public void setClickDelay(boolean clickDelay) {
    ...
    public void twsUpdateItemViewTranslation(int lastPosition, int position, float positionOffset, int index) {
        ...
        if (animationPosition == index) {
            ...
            int minChildCount = getChildCount();
            for (int i = 0; i < getChildCount(); i++) {
                float maxTranslationX = (i + 1) / (float) minChildCount * getWidth();
                child = getChildAt(i);
    ...
    public void twsSetHotspot(Drawable d, float x, float y) {
        try {
			if (d instanceof TwsDrawable) {
				((TwsDrawable) d).twsSetHotspot(x, y);
			}
        } catch (Exception e) {
    ...
    boolean twsTrackMotionScroll(int deltaY, int incrementalDeltaY, int childCount) {
        if (childCount == getCount()) {
            int totalHeight = 0;
            totalHeight = getDisplayItemTotalHeight(childCount);
            if (totalHeight <= getHeight()) {
        ...
        if (isOutOfTop || isOutOfBottom) {
            if (mTouchMode == TOUCH_MODE_SCROLL) {
                incrementalDeltaY /= (1.7f + Math.abs(firstTop / 400.0f));
            }
    ...
    public void setBlurRecycleFlag(boolean flag) {
    	mBlurRecycleFlag = flag;
    }
    
    public void setTouchOffsetFlag(boolean flag) {
    	mIsTouchOffsetFlag = flag;
    }
    
    public void smoothScrollToPositionFromTopWithDuration(int position, int offset, int duration) {
        if (mPositionScroller == null) {
            mPositionScroller = new PositionScroller();
        }
        mPositionScroller.startWithOffset(position, offset, duration);
