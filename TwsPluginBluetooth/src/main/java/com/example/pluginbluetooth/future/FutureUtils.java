...
public class FutureUtils {
    ...
    public static <K, V> Future<Map<K, V>> unwrap(Map<K, Future<V>> futures) {
        final List<Future<Entry<K, V>>> entries = new ArrayList<Future<Entry<K, V>>>();
        for (Map.Entry<K, Future<V>> entry : futures.entrySet()) {
            entries.add(unwrap(entry));
        }
        return merge(entries)
                .map(new MapCallback<List<Entry<K, V>>, Map<K, V>>() {
                    @Override
                    public Map<K, V> onResult(final List<Entry<K, V>> result) {
