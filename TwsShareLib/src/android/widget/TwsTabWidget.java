 ...
 * {@link com.tencent.tws.assistant.widget.TabHost TabHost}. When the user selects a tab, this
 ...
 */
public class TwsTabWidget extends TwsMaxHeightLinearLayout implements OnFocusChangeListener {
    ...
    private OnTabItemCenterPosListener mPosListener = null;

    public interface OnTabItemCenterPosListener {
        public void setTabsPos(int[] positions);
    }
    
    public void setOnTabItemCenterPosListener(OnTabItemCenterPosListener posListener) {
    	mPosListener = posListener;
    }
    
    public TwsTabWidget(Context context) {
        this(context, null);
    }

    public TwsTabWidget(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.tabWidgetStyle);
    }

    public TwsTabWidget(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.TabWidget, defStyle, 0);
    ...
    void measureChildBeforeLayout(View child, int childIndex,
        ...
        super.twsMeasureChildBeforeLayout(child, childIndex,
               widthMeasureSpec, totalWidth, heightMeasureSpec, totalHeight);
    }

    void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
        if (MeasureSpec.getMode(widthMeasureSpec) == MeasureSpec.UNSPECIFIED) {
            super.twsMeasureHorizontal(widthMeasureSpec, heightMeasureSpec);
            return;
        }
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		boolean needSetTOListener = false;
		if (mTabItemCenterPosX == null || mTabItemCenterPosX.length != getChildCount()) {
			mTabItemCenterPosX = new int[getChildCount()];
		}
		...
		for (int index = 0; index < getChildCount(); index++) {
			...
			// note:这里没做类型判断
			centerPosX = (view.getLeft() + view.getRight()) / 2;
			if (mTabItemCenterPosX[index] != centerPosX) {
				mTabItemCenterPosX[index] = centerPosX;
				if (0 < centerPosX)
					needSetTOListener = true;
			}
		}

		if (needSetTOListener)
			mPosListener.setTabsPos(mTabItemCenterPosX);
	}
}
