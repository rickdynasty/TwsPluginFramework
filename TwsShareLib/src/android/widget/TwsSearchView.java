...
import static android.widget.TwsSuggestionsAdapter.getColumnString;
 ...
 */
public class TwsSearchView extends LinearLayout implements CollapsibleActionView, 
		TwsStretchAnimation.AnimationListener{

    private static final boolean DBG = false;
    private static final String LOG_TAG = "TwsSearchView";
    ...
    private TwsSearchAutoComplete mQueryTextView;
    ...
    // tws-start::add cancel click listener 20141223
    // screent width and height
 	private int screentWidth = 0;
 	private int screentHeight = 0;
 	private float screenDensity = 0;
 	private int maxSizeOffset = 24;
 	private int minSizeOffset = 72;
 	// view be able to flexible maxsize 
 	private int maxSize;
 	// view be able to flexible minsize
 	private int minSize;
 	// cuurenr click view
 	private View currentView;
 	// show longest view
 	private View preView;
 	// main layout viewgroup
 	private LinearLayout mainContain;
 	private TwsStretchAnimation stretchanimation;
 	boolean isCancelClick = false;
 	boolean isSearchClick = true;
 	boolean isSearchMode = false;
 	boolean isEnable = true;
 	boolean isReturnEvent = false;
    // tws-start::add cancel click listener 20142116
    ...
    private Runnable mShowImeRunnable = new Runnable() {
        public void run() {
            ...
            if (imm != null) {
                HIDDEN_METHOD_INVOKER.showSoftInputUnchecked(imm, TwsSearchView.this, 0);
            }
        }
    };
    ...
    // tws-start::add cancel click listener 20141023
    public interface onCancelClickListener{
    	// user listener cancel click event
    	public void onClick(boolean isOnCancelClick);
    }
    ...
    public void setOnCancelClickListener (final onCancelClickListener listener) {
    	mOnCancelClickListener = listener;
    }
    
    public void setOpenFoucusable(boolean isFouces){
    	// reconsitution code not need setting edittext focus
    }
    
   public void setSearchViewBackgroundResource(int resId) {
	   mSearchBar.setBackgroundResource(resId);
//	   mSearchEditFrame.setBackgroundResource(resId);
   }
   ...
   public void setSearchViewBackground(Drawable drawable) {
	   if (android.os.Build.VERSION.SDK_INT > 15) {
		   ...
		   mSearchBar.setBackgroundDrawable(drawable);
	   }
//	   mSearchEditFrame.setBackground(drawable);
   }
   
   public void setSearchViewBackgroundColor(int color) {
//	   mSearchEditFrame.setBackgroundColor(color);
	   mSearchBar.setBackgroundColor(color);
   }
   
   public void setSearchEditTextHint(String str) {
	// reconsitution code not need setting edittext hint
   }
   
   public void setSearchEditTextHint(int resid) {
	   
   }																																					
   
   public void setSearchEditHintResource(int resid) {
	   mQueryTextView.setHintTextColor(mContext.getResources().getColorStateList(resid));
   }
   
   public void setSearchIconResource(int resid) {
	   mQueryTextView.setCompoundDrawablesWithIntrinsicBounds(mContext.getResources().getDrawable(resid),
			   null, null, null);
   }
   
   public void setSearchCancelButton(String str) {
	   mSearchCancelButton.setText(str);
   }
   
   
   public void setSearchEditBackgroundResource(int resid) {
	   mSearchPlate.setBackgroundResource(resid);
   }
   
   public void setSearchEditBackgroundColor(int color) {
	   mSearchPlate.setBackgroundColor(color);
   }
   
   public void setSearchViewDefaultStatus() {
	   if(mSearchCancelButton.getVisibility() == VISIBLE) {
		   onCancelClicked();
	   }
   }
   ...
   public void setSearchViewIsChange(boolean change) {
	   isChange = change;
   }
   
   public void setSearchViewThemeDarkToLight() {
	   // drawable
	   mSearchBar.setBackgroundResource(R.drawable.search_view_light_background);
	   mSearchPlate.setBackgroundColor(mContext.getResources().getColor(R.color.search_view_editor_light_background));
	   // textcolor
	   mQueryTextView.setTextColor(mContext.getResources().getColor(R.color.tws_light_body));
	   mQueryTextView.setHintTextColor(mContext.getResources().getColor(R.color.search_view_text_color_hint_light));
	   mSearchCancelButton.setTextColor(mContext.getResources().getColor(R.color.tws_text_blue));
	   // padding drawable
	   mCloseButton.setImageResource(R.drawable.second_ic_clear_holo_light);
	   mQueryTextView.setCompoundDrawablesWithIntrinsicBounds(mContext.getResources().getDrawable(R.drawable.ic_search),
			   null, null, null);
	   mQueryTextView.twsSetCursorDrawableRes(R.drawable.search_view_cursor_drawable_light);
   }
   
   public void setSearchViewHeightAndMargin(int height,int topMargin, int bottomMargin){
	   LinearLayout.LayoutParams para = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, height);
	   mSearchBar.setLayoutParams(para);
	   mSearchBar.setPadding(0, topMargin, 0, bottomMargin);
	   requestLayout();
   }
  ...
  // tws-end::add cancel click listener 20141023
    /**
    ...
    public TwsSearchView(Context context) {
        this(context, null);
    }

    public TwsSearchView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.searchViewStyle);
    }

    public TwsSearchView(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }
  
    public TwsSearchView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
//        super(context, attrs);
        ...
        mQueryTextView = (TwsSearchAutoComplete) findViewById(R.id.search_src_text);
        mQueryTextView.setFocusable(false);
        mQueryTextView.setFocusableInTouchMode(false);

        mQueryTextView.clearFocus();
        mSearchCancelButton = (TextView) findViewById(R.id.cancel_search_btn);
		boolean bRipple = ThemeUtils.isShowRipple(context);
		if(bRipple){
        	TwsRippleDrawable mCancelBtnRippleDrawable = TwsRippleUtils.getDefaultDarkDrawable(mContext);
//        mCancelBtnRippleDrawable.twsSetHotspotBounds(0, 0, 300, 300);
        	if (android.os.Build.VERSION.SDK_INT > 15) {
        		mSearchCancelButton.setBackground(mCancelBtnRippleDrawable);
        	}
        	else {
        		mSearchCancelButton.setBackgroundDrawable(mCancelBtnRippleDrawable);
        	}
		}
//        TwsRippleEffectInterface.setupEffectionOverlay(mSearchCancelButton, mContext);
        ...
        mSearchCancelButton.setOnClickListener(mOnClickListener);
        
        mQueryTextView.setOnLongClickListener(new OnLongClickListener() {
			@Override
			public boolean onLongClick(View arg0) {
				Log.i(LOG_TAG, "OnLongClickListener mQueryTextView 11111");
				if(mSearchCancelButton.getVisibility() == VISIBLE) {
					Log.i(LOG_TAG, "OnLongClickListener mQueryTextView 22222");
					mQueryTextView.setCursorVisible(true);
					return false;
				}else {
					Log.i(LOG_TAG, "OnLongClickListener mQueryTextView 33333");
        ...
        mQueryTextView.setOnFocusChangeListener(new OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if (mOnQueryTextFocusChangeListener != null) {
                    mOnQueryTextFocusChangeListener.onFocusChange(TwsSearchView.this, hasFocus);
                }
            }
        });
        // bruce
        mainContain = mSearchEditFrame;
        initCommonData();
		initViewData(0);
		// bruce
		Log.i(LOG_TAG, "obtainStyledAttributes 11111");
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SearchView, defStyleAttr, 0);
        Log.i(LOG_TAG, "obtainStyledAttributes 22222 a = " + a.toString());
        // tws-start::by bruce::init searchview whether collapse one icon::2014-12-14
        setIconifiedByDefault(a.getBoolean(R.styleable.SearchView_iconifiedByDefault, false));
        // tws-end::by bruce::init searchview whether collapse one icon::2014-12-14
        int maxWidth = a.getDimensionPixelSize(R.styleable.SearchView_maxWidth, -1);
        Log.i(LOG_TAG, "obtainStyledAttributes 33333 maxWidth = " + maxWidth);
        if (maxWidth != -1) {
            setMaxWidth(maxWidth);
        }
        CharSequence queryHint = a.getText(R.styleable.SearchView_queryHint);
        Log.i(LOG_TAG, "obtainStyledAttributes 44444 queryHint = " + queryHint);
        if (!TextUtils.isEmpty(queryHint)) {
            setQueryHint(queryHint);
        }
        int imeOptions = a.getInt(R.styleable.SearchView_android_imeOptions, -1);
        Log.i(LOG_TAG, "obtainStyledAttributes 55555 imeOptions = " + imeOptions);
        if (imeOptions != -1) {
            setImeOptions(imeOptions);
        }
        int inputType = a.getInt(R.styleable.SearchView_android_inputType, -1);
        Log.i(LOG_TAG, "obtainStyledAttributes 66666 inputType = " + inputType);
        if (inputType != -1) {
            setInputType(inputType);
        }
        boolean focusable = true;
        focusable = a.getBoolean(R.styleable.SearchView_focusable, focusable);
        Log.i(LOG_TAG, "obtainStyledAttributes 77777 focusable = " + focusable);
...
//        setFocusable(focusable);
    ...
    public boolean requestFocus(int direction, Rect previouslyFocusedRect) {
        ...
        if (!isIconified()) {
            ...
            if (result) {
            	// resolve requese fouces problem because mms
//                updateViewsVisibility(false);
    ...
    public void clearFocus() {
        ...
        super.clearFocus();
        mQueryTextView.clearFocus();
        // tws-start::by bruce::init twssearchview status::2014-12-18
        mQueryTextView.setFocusable(false);
        mQueryTextView.setFocusableInTouchMode(false);
        // tws-start::by bruce::init twssearchview status::2014-12-18
    ...
    public void setQueryRefinementEnabled(boolean enable) {
        ...
        if (mSuggestionsAdapter instanceof TwsSuggestionsAdapter) {
            ((TwsSuggestionsAdapter) mSuggestionsAdapter).setQueryRefinement(
                    enable ? TwsSuggestionsAdapter.REFINE_ALL : TwsSuggestionsAdapter.REFINE_BY_ENTRY);
        }
    }
    ...
    private void updateViewsVisibility(final boolean collapsed) {
        ...
        if((mSearchCancelButton.getVisibility() == VISIBLE) && (collapsed == false)) {
        	onCancelClicked();
        }
        
        // modify frame whether show?
        mSearchEditFrame.setVisibility(collapsed ? GONE : VISIBLE);
        mSearchPlate.setVisibility(collapsed ? GONE : VISIBLE);
        mSearchCancelButton.setVisibility(collapsed ? VISIBLE : GONE);
        updateCloseButton();
    ...
    private void updateSubmitButton(boolean hasText) {
        ...
        // mSubmitButtonEnabled: by setSubmitButtonEnabled() method setting
        // ever is false,In our the ROM not use this para
    ...
    private final OnClickListener mOnClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            if(v == mCloseButton) {
            	...
            	Log.i(LOG_TAG, "isEnable = " + isEnable + "; isSearchClick = " + isSearchClick + 
            			"; isReturnEvent = " + isReturnEvent);
            	mQueryTextView.setCursorVisible(true);
            	if(isEnable && isSearchClick) {
            		onSearchClicked();
            		forceSuggestionQuery();
            	}else if(isReturnEvent && isEnable) {
            		// bruce
...
//                    }
            ...
            } else if(v == mSearchCancelButton) {
            	Log.i(LOG_TAG, "isCancelClick = " + isCancelClick);
    ...
    private static String getActionKeyMessage(Cursor c, SearchableInfo.ActionKeyInfo actionKey) {
        ...
        if (column != null) {
            result = TwsSuggestionsAdapter.getColumnString(c, column);
        }
...
//		//geofffeng ImageSpan align bottom, the icon is not larger than font, so not need to *1.25
...
//        ssb.setSpan(new ImageSpan(searchIcon), 1, 2, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    private void updateSearchAutoComplete() {
        ...
        if (mSearchable.getSuggestAuthority() != null) {
            mSuggestionsAdapter = new TwsSuggestionsAdapter(getContext(),
                    this, mSearchable, mOutsideDrawablesCache);
            mQueryTextView.setAdapter(mSuggestionsAdapter);
            ((TwsSuggestionsAdapter) mSuggestionsAdapter).setQueryRefinement(
                    mQueryRefinement ? TwsSuggestionsAdapter.REFINE_ALL
                    : TwsSuggestionsAdapter.REFINE_BY_ENTRY);
        }
    }
...
//        mVoiceButton.setVisibility(visibility);
    ...
    private void onTextChanged(CharSequence newText) {
        ...
        updateCloseButton();
        updateSubmitArea();
        // tws-start::by bruce::control mSearchCancelButton status::2014-12-14
//        if(mSearchCancelButton.getVisibility() != VISIBLE) {
//        	mSearchCancelButton.setVisibility(VISIBLE);
//        }
        // tws-end::by bruce::control mSearchCancelButton status::2014-12-14
    ...
    // tws-start add search cancel fun::20141001
    private void onCancelClicked() {
    	 // bruce
//    	 isSearchClick = true;
    	 isCancelClick = false;
    	 if(!isSearchMode) {
    		 btnDisappearAnimator();
    	 }else {
    		 // tws-start::by bruce::setShowSearchMode mSearchCancelButton status::2015-01-14
 	        if(mSearchCancelButton.getVisibility() == VISIBLE) {
 	        	mSearchCancelButton.setVisibility(GONE);
 	        	isSearchClick = true;
 	        }
 	       // tws-end::by bruce::setShowSearchMode mSearchCancelButton status::2015-01-14
    	 }
		 // bruce
    	 CharSequence text = mQueryTextView.getText();
...
//    	 updateViewsVisibility(false);
    	 if(mOnCancelClickListener != null) {
    		 mOnCancelClickListener.onClick(true);
    	 }
    }
    // tws-end add search cancel fun::20141001
...
//        updateViewsVisibility(false);
    ...
    public void onAnimatorByClick() {
//    	isCancelClick = true;
    	currentView = mainContain.getChildAt(0);
		clickEvent(currentView);
//		onOffClickable(false);
		stretchanimation.startAnimation(currentView, 1);
    }
    
    private void onSearchviewAnimatorByClick() {
    	
    }
    
	private void onSearchviewAnimatorByCancelClick() {
...
//        updateViewsVisibility(isIconified());
    ...
    private final OnItemSelectedListener mOnItemSelectedListener = new OnItemSelectedListener() {
        ...
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            ...
            TwsSearchView.this.onItemSelected(position);
        }
    ...
    private TextWatcher mTextWatcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start,
            ...
            TwsSearchView.this.onTextChanged(s);
        }
     ...
     * @hide
     */
    public static class TwsSearchAutoComplete extends AutoCompleteTextView {

        private int mThreshold;
        private TwsSearchView mSearchView;

        public TwsSearchAutoComplete(Context context) {
            super(context);
            mThreshold = getThreshold();
        }

        public TwsSearchAutoComplete(Context context, AttributeSet attrs) {
            super(context, attrs);
            mThreshold = getThreshold();
        }

        public TwsSearchAutoComplete(Context context, AttributeSet attrs, int defStyle) {
            super(context, attrs, defStyle);
            mThreshold = getThreshold();
        }

        void setSearchView(TwsSearchView searchView) {
    	...
    	public void twsSetCursorDrawableRes(int resId){
            try {
                Class c = ReflectUtils.forClassName("android.widget.TextView");
                Field mCursorDrawableRes = ReflectUtils.getDeclaredField(TextView.class, "mCursorDrawableRes");
                ReflectUtils.setFieldValue((TextView)this, mCursorDrawableRes, resId);
            } catch (Exception e) {
            	e.printStackTrace();
            }
    	}
        ...
        public boolean onKeyPreIme(int keyCode, KeyEvent event) {
        	InputMethodManager imm = (InputMethodManager)
                     getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
            if ((imm != null) && (event.getAction() == 0)) {
                isShow = imm.hideSoftInputFromWindow(getWindowToken(), 0);
                Log.i("TwsSearchView", "onKeyPreIme isShow = " + isShow  + "; event = " + event.getAction());
            }
...
//                    mSearchView.isSearchClick = true;
                    mSearchView.isReturnEvent = true;
                    if(mSearchView.isChange && (mSearchView.mSearchCancelButton.getVisibility() == VISIBLE)) {
    ...
    private static class AutoCompleteTextViewReflector {
		...
		AutoCompleteTextViewReflector() {
			doBeforeTextChanged = ReflectUtils.getDeclaredMethod(
					AutoCompleteTextView.class, "doBeforeTextChanged");
			doAfterTextChanged = ReflectUtils.getDeclaredMethod(
					AutoCompleteTextView.class, "doAfterTextChanged");
			ensureImeVisible = ReflectUtils.getDeclaredMethod(
					AutoCompleteTextView.class, "ensureImeVisible",
					boolean.class);
			showSoftInputUnchecked = ReflectUtils.getDeclaredMethod(
					InputMethodManager.class, "showSoftInputUnchecked",
    ...
    private void initCommonData() {
		...
		// horizatal
		measureSize(screentWidth, screenDensity);
		stretchanimation = new TwsStretchAnimation(maxSize, minSize,
				TwsStretchAnimation.TYPE.horizontal, 100);
		stretchanimation.setInterpolator(new AccelerateInterpolator());
		stretchanimation.setDuration(100);
...
//			child.setOnClickListener(this);
			...
			if (stretchanimation.getmType() == TwsStretchAnimation.TYPE.horizontal) {
				params.width = sizeValue;
			} else if (stretchanimation.getmType() == TwsStretchAnimation.TYPE.vertical) {
	 ...
	 * @param index
	 */
	private void measureSize(int layoutSize, float density) {
		Log.i(LOG_TAG, "density = " + density);
		int halfWidth = layoutSize / 2;
		maxSize = layoutSize - maxSizeOffset * (int)density;		
		String country = mContext.getResources().getConfiguration().locale.getCountry();
		boolean isUKorUS = country.equals("UK") || country.equals("US");
		minSize = layoutSize - (isUKorUS?minSizeOffset + 16:minSizeOffset) * (int)density;
		Log.i(LOG_TAG,  "country="+country + " maxWidth=" + maxSize + " minWidth = " + minSize);
	}
	
	// LinearLayout all childView is click
	...
	public void animationEnd(View v, int type) {
		...
		Log.e(LOG_TAG, "animationEnd tpye = " + type);
		if(type == 1) {
			btnAppearAnimator();
		}else{
			isSearchClick = true;
	...
	public void btnAppearAnimator(){
//		mSearchCancelButton.setVisibility(View.VISIBLE);
		PropertyValuesHolder pvalphaCancel = PropertyValuesHolder.ofFloat("alpha", 0.0f,1.0f);
		float getx = mSearchCancelButton.getX();
		float getWidth = mSearchCancelButton.getWidth();
    	PropertyValuesHolder pvxCancel = PropertyValuesHolder.ofFloat("translationX",
    			mSearchCancelButton.getX() + mSearchCancelButton.getWidth(), mSearchCancelButton.getX());
    
    	ObjectAnimator mObjectAnimator = ObjectAnimator.ofPropertyValuesHolder(mSearchCancelButton, pvalphaCancel/*, pvxCancel*/);
    	mObjectAnimator.addListener(new AnimatorListener(){
			...
			public void onAnimationStart(Animator arg0) {
				// TODO Auto-generated method stub
//				mSearchCancelButton.setVisibility(View.VISIBLE);
//				mSearchCancelButton.setText("aaaaaaa");
				 // tws-start::by bruce::control mSearchCancelButton status::2014-12-14
		        if(mSearchCancelButton.getVisibility() != VISIBLE) {
		        	mSearchCancelButton.setVisibility(VISIBLE);
		        }
	...
	public void btnDisappearAnimator(){
		PropertyValuesHolder pvalphaCancel = PropertyValuesHolder.ofFloat("alpha", 1.0f,0.0f);
    	PropertyValuesHolder pvxCancel = PropertyValuesHolder.ofFloat("translationX",
    			mSearchCancelButton.getX(), mSearchCancelButton.getX()+ mSearchCancelButton.getWidth());
...
//				mSearchCancelButton.setVisibility(View.GONE);
		        ...
		        stretchanimation.startAnimation(currentView, 2);
		       // tws-end::by bruce::control mSearchCancelButton status::2014-12-14
			}
