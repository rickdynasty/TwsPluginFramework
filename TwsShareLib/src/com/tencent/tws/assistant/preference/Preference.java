...
public class Preference implements Comparable<Preference> {
	...
	protected boolean mRightIcon = false;
    protected boolean mRippleEffect = true;
    /**
    ...
    public static final int GROUP_PREFERENCE    = 0;
    public static final int FRIST_PREFERENCE    = 1;
    public static final int NORMAL_PREFERENCE   = 2;
    public static final int LAST_PREFERENCE     = 3;
    public static final int SINGLE_PREFERENCE   = 4; 
    public static final int ONE_PREFERENCE      = 5;
    public static final int TOP_PREFERENCE      = 6;
	private ImageView mRightIconView;
	private CharSequence mCaption;
	private int mRightIconResId;
    private Drawable mRightIconDrawable;
	private OnPreferenceRightIconClickListener mRightIconClickListener;
    ...
    public Preference(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray t = context.obtainStyledAttributes(attrs,
                R.styleable.twsPreference, defStyle, 0);
		mRightIcon = t.getBoolean(R.styleable.twsPreference_preferenceArrow, false);
		mRippleEffect = t.getBoolean(R.styleable.twsPreference_preferenceRippleEffect, true);
		mRightIconDrawable = mContext.getResources().getDrawable(R.drawable.tws_expand_arrow_right);
 		t.recycle();
 		
        TypedArray a = context.obtainStyledAttributes(attrs,
                android.R.styleable.Preference, defStyle, 0);
        for (int i = a.getIndexCount(); i >= 0; i--) {
            int attr = a.getIndex(i); 
            switch (attr) {
                case android.R.styleable.Preference_icon:
                    mIconResId = a.getResourceId(attr, 0);
                    break;

                case android.R.styleable.Preference_key:
                    mKey = a.getString(attr);
                    break;
                    
                case android.R.styleable.Preference_title:
                    mTitleRes = a.getResourceId(attr, 0);
                    mTitle = a.getString(attr);
                    break;
                    
                case android.R.styleable.Preference_summary:
                    mSummary = a.getString(attr);
                    break;
                    
                case android.R.styleable.Preference_order:
                    mOrder = a.getInt(attr, mOrder);
                    break;

                case android.R.styleable.Preference_fragment:
                    mFragment = a.getString(attr);
                    break;

                case android.R.styleable.Preference_layout:
                    mLayoutResId = a.getResourceId(attr, mLayoutResId);
                    break;

                case android.R.styleable.Preference_widgetLayout:
                    mWidgetLayoutResId = a.getResourceId(attr, mWidgetLayoutResId);
                    if(Build.VERSION.SDK_INT <= Build.VERSION_CODES.JELLY_BEAN_MR2){
//                    	mWidgetLayoutResId = R.layout.preference_widget_switch_jb;
                    }
                    break;
                    
                case android.R.styleable.Preference_enabled:
                    mEnabled = a.getBoolean(attr, true);
                    break;
                    
                case android.R.styleable.Preference_selectable:
                    mSelectable = a.getBoolean(attr, true);
                    break;
                    
                case android.R.styleable.Preference_persistent:
                    mPersistent = a.getBoolean(attr, mPersistent);
                    break;
                    
                case android.R.styleable.Preference_dependency:
                    mDependencyKey = a.getString(attr);
                    break;
                    
                case android.R.styleable.Preference_defaultValue:
                    mDefaultValue = onGetDefaultValue(a, attr);
                    break;
                    
                case android.R.styleable.Preference_shouldDisableView:
        ...
        if (!getClass().getName().startsWith("android.preference")
                ...
                && !getClass().getName().startsWith("com.tencent.tws")) {
...
//        	mWidgetLayoutResId = R.layout.preference_widget_switch_jb;
    ...
    public boolean getArrowGone() {
        return mRightIcon;
    }
	public void setRippleEffect(boolean b) {
        mRippleEffect = b;
    }
    ...
    public void setArrowIcon(Drawable icon) {
        if ((icon == null && mRightIconDrawable != null) || (icon != null && mRightIconDrawable != icon)) {
            mRightIconDrawable = icon;

            notifyChanged();
        }
    }

    public void setArrowIcon(int iconResId) {
        mRightIconResId = iconResId;
        setArrowIcon(mContext.getResources().getDrawable(iconResId));
    }
    ...
    public void setCaption(CharSequence caption) {
        if (caption == null && mCaption != null || caption != null && !caption.equals(mCaption)) {
            mCaption = caption;
            notifyChanged();
        }
    }

    public void setCaption(int captionResId) {
        setCaption(mContext.getString(captionResId));
    }

	
	public interface OnPreferenceRightIconClickListener {
			boolean onPreferenceRightIconClick(Preference preference);
		}
	
    public void setOnPreferenceRightIconClickListener(OnPreferenceRightIconClickListener listener) {
        mRightIconClickListener = listener;
    }

	private void setHeaderAndFooterVisiable(View header, View footer ,boolean bHeader, boolean bFooter){
		header.setVisibility(bHeader ? View.VISIBLE : View.GONE);
		footer.setVisibility(bFooter ? View.VISIBLE : View.GONE);
	}
	//tws-end 20140721::preference
    ...
    protected View onCreateView(ViewGroup parent) {
        ...
        mRightIconView = (ImageView) layout.findViewById(R.id.arrowicon);
...
//                titleView.setVisibility(View.GONE);
        ...
        final TextView captionView = (TextView) view.findViewById(R.id.caption);
        if (captionView != null) {
        ...
        final ImageView rightIconView = (ImageView) view.findViewById(R.id.arrowicon);
        if (rightIconView != null) {
            if (mRightIconResId != 0 || mRightIconDrawable != null) {
                if (mRightIconDrawable == null) {
                    mRightIconDrawable = getContext().getResources().getDrawable(mRightIconResId);
                }
                if (mRightIconDrawable != null) {
            ...
            if (mRightIconClickListener != null) {
                rightIconView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View arg0) {
                        mRightIconClickListener.onPreferenceRightIconClick(Preference.this);
                    }
                });
            }
            rightIconView.setVisibility((mRightIconDrawable != null && mRightIcon) ? View.VISIBLE
                    : View.GONE);
        }

        final ViewGroup widgetFrame = (ViewGroup) view.findViewById(R.id.widget_frame);
        if (widgetFrame != null) {
            if (mWidgetLayoutResId != 0 && isSupportWidgetFrameClick()) {
                widgetFrame.setFocusable(false);
        ...
        int index = getIndex();
        int mListMaxHeight = (int) mContext.getResources().getDimension(
                R.dimen.tws_listview_item_max_height);
        int mListSmallHeight = (int) mContext.getResources().getDimension(
                R.dimen.tws_listview_item_small_height);
        int mBlankPadding = (int) mContext.getResources().getDimension(
                R.dimen.preference_item_empty_padding);
        int paddingLeft = (int) mContext.getResources().getDimension(
                R.dimen.tws_listview_item_padding_side_left);
...
//        int paddingBottom = view.getPaddingBottom();
        ...
        if (index == GROUP_PREFERENCE) {
            if (bSummary) {
                view.setMinimumHeight(mListMaxHeight);
            } else {
                view.setMinimumHeight(mListSmallHeight);
            }
            view.setPadding(paddingLeft, paddingTop, paddingRight, paddingBottom);
        } else if (index == FRIST_PREFERENCE) {
            if (bSummary) {
                view.setMinimumHeight(mListMaxHeight + mBlankPadding);
            } else {
                view.setMinimumHeight(mListSmallHeight + mBlankPadding);
            }
            view.setPadding(paddingLeft, paddingTop + mBlankPadding, paddingRight, paddingBottom);
        } else if (index == NORMAL_PREFERENCE) {
            if (bSummary) {
                view.setMinimumHeight(mListMaxHeight);
            } else {
                view.setMinimumHeight(mListSmallHeight);
            ...
            view.setPadding(paddingLeft, paddingTop, paddingRight, paddingBottom);
        } else if (index == SINGLE_PREFERENCE) {
            if (bSummary) {
                view.setMinimumHeight(mListMaxHeight + mBlankPadding * 2);
            } else {
                view.setMinimumHeight(mListSmallHeight);
            }
            view.setPadding(paddingLeft, paddingTop + mBlankPadding, paddingRight, paddingBottom);
        } else if (index == ONE_PREFERENCE) {
            if (bSummary) {
                view.setMinimumHeight(mListMaxHeight + mBlankPadding * 2);
            } else {
                view.setMinimumHeight(mListSmallHeight);
        ...
        if (!mRippleEffect) {
            view.setTag("noShowRippleEffect");
        }
    }
