	...
	synchronized InstallResult installPlugin(String srcPluginFile) {
		...
		if (oldPluginDescriptor != null) {
			...
			if (PluginLauncher.instance().isRunning(oldPluginDescriptor.getPackageName())) {
				if (oldPluginDescriptor.getVersionCode() < pluginDescriptor.getVersionCode()) {
					TwsLog.w(TAG, "旧版插件已经加载， 且插件版本 高于 旧插件版本 —— 直接删除旧版，进行热更新");
					remove(oldPluginDescriptor.getPackageName());
				} else {
					Toast.makeText(PluginLoader.getApplication(), "新插件版本不高于旧插件版本，拒绝安装!", Toast.LENGTH_SHORT).show();
					TwsLog.w(TAG, "旧版插件已经加载， 且新插件版本不高于旧插件版本，拒绝安装");
					new File(srcPluginFile).delete();
